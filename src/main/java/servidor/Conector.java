...
public class Conector {
	...
	// public boolean getLoguearUsuario(PaqueteUsuario user) {
...
//			}
	...
	public PaqueteUsuario getUsuario(String usuario) {
		...
		user = session.createQuery(userQuery).getSingleResult();
			...
			Servidor.getLog().append(he.getMessage() + System.lineSeparator());
		}
		return user;
	}
//	ResultSet result = null;
	...
	int i = 2 , j = 1;
		try {
			...
			if(mochila != null) {
				...
				Servidor.getLog().append("No se encontro la mochila del personaje " + paquetePersonaje.getNombre() + System.lineSeparator());
			}
